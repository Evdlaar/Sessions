...
License: 
	This demo script, that is part of the Efficient Indexing Strategies 
...
-- the AdventureWorks2014 database so we have
...
-- Run Ostress to insert 100.000 rows into the table
-- "C:\Program Files\Microsoft Corporation\RMLUtils\ostress.exe" -E -dIndex_Test -i"c:\insert_rows_IndexKeySmall.sql" -n200 -r500 -q
...
FROM sys.dm_db_index_physical_stats
    (DB_ID(N'Index_Test'), OBJECT_ID(N'IndexKeySmall'), NULL, NULL , 'DETAILED');
...
-- Run Ostress to insert 100.000 rows into the table
-- "C:\Program Files\Microsoft Corporation\RMLUtils\ostress.exe" -E -dIndex_Test -i"c:\insert_rows_IndexKeyLarge.sql" -n200 -r500 -q
-- Takes around 1 minute
...
SELECT  
  ...
  record_count
FROM sys.dm_db_index_physical_stats
    (DB_ID(N'Index_Test'), OBJECT_ID(N'IndexKeyLarge'), NULL, NULL , 'DETAILED');
...
-- Run the previous query, it has a missing index
SELECT 
  AddressLine1, 
  AddressLine2, 
  City, 
